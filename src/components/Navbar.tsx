...
import { useEffect, useRef, useState } from "react";
import { smartNavigate, detectDevice } from "@/lib/smartStore";
...
const APP_SCHEME = undefined; // e.g. "glimvia://open"
const NAV_UTM = "utm_source=nav&utm_medium=cta";
...
export default function Navbar() {
  ...
  // Recalculate width when theme changes (logos swap)
  useEffect(() => {
    measureAndSetWidth(collapsedIdle);
  }, [isDark, collapsedIdle]);
  ...
  return (
    <>
      <header
        ...
        aria-label="Primary"
        onMouseEnter={() => {
          overNavRef.current = true;
          cancelIdle();
        }}
        onMouseLeave={() => {
          overNavRef.current = false;
          armIdleTimer();
        }}
      >
        <nav className="flex items-center justify-between gap-3">
          {/* THEME-AWARE LOGO MARK + WORDMARK (links home) */}
          <a
            ref={brandRef}
            href="/"
            aria-label="Glimvia — back to home"
            className="inline-flex items-center gap-2"
            title="Glimvia — Home"
          >
            <span className="sr-only">Glimvia</span>
            {/* light theme logo */}
            <img
              src="/glimvia-logo-light.svg"
              alt=""
              className="h-7 md:h-8 object-contain dark:hidden"
              onLoad={() => measureAndSetWidth(collapsedIdle)}
            />
            {/* dark theme logo */}
            <img
              src="/glimvia-logo-dark.svg"
              alt=""
              className="h-7 md:h-8 object-contain hidden dark:block"
              onLoad={() => measureAndSetWidth(collapsedIdle)}
            />
            {/* wordmark */}
            <span className="ml-1 text-base sm:text-lg font-semibold tracking-tight text-slate-900 dark:text-white">
              Glimvia
            </span>
          </a>
          ...
          <ul
            className={cn(
              ...
              collapsedIdle && "opacity-0 pointer-events-none"
            )}
          >
            {LINKS.map((l) => (
              <li key={l.href}>
                <a
                  href={l.href}
                  className="link-ux opacity-90 hover:opacity-100 transition"
                >
                  {l.label}
                </a>
              </li>
          ...
          >
            ...
            >
              {!mounted ? (
                <span
                  aria-hidden
                  className="block size-4 rounded-full bg-slate-400/60"
                />
              ) : isDark ? (
                <SunIcon />
              ) : (
          ...
          <button
            aria-label="Open menu"
            className={cn(
              "md:hidden size-10 rounded-full glass flex items-center justify-center active:scale-[.98] transition",
              collapsedIdle && "opacity-0 pointer-events-none"
            )}
            onClick={() => setMobileOpen(true)}
          >
        ...
        <div className="absolute left-1/2 -translate-x-1/2 top-[18vh] w-[min(92vw,520px)] text-center">
          ...
          <div className="mt-8">
            ...
            >
              {mounted ? (isDark ? <SunIcon /> : <MoonIcon />) : (
                <span className="size-4 rounded-full bg-slate-400/60" />
              )}
          ...
          <div className="mt-6">
            ...
            <a
              ...
              className={cn("inline-block", ctaClass)}
            >
              Get the app
            </a>
          </div>
