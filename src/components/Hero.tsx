...
import clsx from "clsx";
import { ArrowUpRight, Gift, Zap, Scaling } from "lucide-react"; // Existing icons
import { FaApple, FaGooglePlay } from "react-icons/fa"; // Importing new icons from react-icons/fa
...
const TAGS = [
  ...
  { text: "Easily extended to meet your needs", icon: <Scaling size={16} /> },
];

export default function Hero() {
  // Common button classes for consistent sizing and styling
  const buttonClasses = "glow-cta cta-shine glass inline-flex items-center justify-center h-14 rounded-full px-6 text-[0.98rem] font-medium shadow-sm hover:opacity-95 active:scale-[.98]";
  ...
  return (
    <section id="cta" className="relative mx-auto min-h-screen max-w-6xl px-4 sm:px-6">
      <div className="flex h-screen flex-col items-center justify-center">
        ...
        <p className="mx-auto mt-8 max-w-3xl text-center text-[1.05rem] text-slate-700 dark:text-slate-300/80">
          Your dashboards, KPIs and alerts now live in your pocket so decisions never have to wait.
        </p>
        <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
          <a href="#" className={buttonClasses}>
            <FaApple size={18} /> {/* Apple icon from react-icons */}
            <span>Download on App&nbsp;Store</span>
          </a>
          <a href="#" className={buttonClasses}>
            <FaGooglePlay size={18} /> {/* Google Play icon from react-icons */}
            <span>Get it on Google&nbsp;Play</span>
          </a>
        ...
        <div className="mx-auto mt-12 w-full max-w-5xl">
          <div className="ticker glass rounded-full py-3 px-4">
            <div className="ticker-track">
              {/* We duplicate the content for a seamless loop */}
              {[...TAGS, ...TAGS].map((tag, index) => (
                <div key={index} className="flex items-center gap-2 mx-4">
                  {tag.icon}
                  <span className="text-sm text-slate-700 dark:text-slate-200/90 whitespace-nowrap">
                    {tag.text}
                  </span>
                </div>
              ))}
